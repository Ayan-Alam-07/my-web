...
const FinalRedeem = ({
  ...
  const modalRef = useRef(null);
  const { user, fetchCoins, setIsRedeemHistory } = useList();
  ...
  const handleRedeem = async (e) => {
    ...
    if (!selectedVoucher) {
      return showError("No voucher selected");
    }

    if (userCoins < selectedVoucher.requiredCoins) {
      return showError(
        `Insufficient balance. You need ${selectedVoucher.requiredCoins.toLocaleString()} VEs to redeem this voucher.`,
      );
    }
    ...
    if (name === "Upi" && !upiId.trim()) {
      return showError("Please enter your UPI ID");
    }

    if ((name === "usdtGlob" || name === "btcGlob") && !binanceId.trim()) {
      return showError("Please enter your Binance Pay ID");
    }

    if (!["usdtGlob", "btcGlob", "Upi"].includes(name) && !email.trim()) {
    ...
    try {
      ...
      const payload = {
        voucherId: selectedVoucher._id || selectedVoucher.id,
        payoutType,
        fullName,
        upiId,
        email,
                    ...
                    <div className="col-11">
                      <form onSubmit={handleRedeem}>
                        ...
                        >
                          <button
                            type="submit"
                            className={style.confirmRedeemBtn}
                            disabled={loading}
                          >
